...
The library exposes a mock data object and a `getMockedDataset` utility function for testing purposes:

```ts
import { getMockedDataset } from '@h5web/lib';

const dataset = getMockedDataset('/nD/twoD');

const values = dataset.value.flat(Infinity) as number[];
const dataArray = ndarray<number>(values, dataset.dims);
```
