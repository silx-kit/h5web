...
- **`getDomain(values: ndarray | number[], scaleType: ScaleType = ScaleType.Linear): Domain | undefined`** - Find the min and max values contained in a array (or ndarray) of numbers. If `scaleType` is `ScaleType.Log` and the domain crosses zero, clamp the min to the first positive value or return `undefined` if the domain is not supported (i.e. `[-x, 0]`).
- **`useDomain(...args): Domain | undefined`** - Memoised React hook wrapper of `getDomain`.
- **`getDomains(arrays: (ndarray | number[])[], scaleType: ScaleType = ScaleType.Linear): (Domain | undefined)[]`** - Find the domains of multiple arrays (or ndarray) of numbers. Useful when dealing with auxiliary curves.
- **`useDomains(...args): (Domain | undefined)[]`** - Memoised React hook wrapper of `getDomains`.
- **`getCombinedDomain(domains: (Domain | undefined)[]): Domain | undefined`** - Combine multiple domains into one. Useful to find the overall domain of a line visualization with auxiliary curves.
- **`useCombinedDomain(...args): Domain | undefined`** - Memoised React hook wrapper of `getCombinedDomain`.
