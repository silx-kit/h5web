...
A library of React components and utilities for data visualization, based on [react-three-fiber](https://github.com/pmndrs/react-three-fiber).
...
The library exposes a mock data object for testing purposes:

```ts
import { mockData } from '@h5web/lib';

const dataset = mockData.datasets['c8f60c29-aae2-11ea-84a9-b94ddd2ec9e8'];

const values: number[] = dataset.value.flat(Infinity);
const dataArray = ndarray<number>(values, dataset.shape.dims);
```
