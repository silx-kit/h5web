...
Find the domains of multiple arrays (or ndarray) of numbers. Useful when dealing with auxiliary curves.
...
Extend a domain by a given factor.

```ts
extendDomain(bareDomain: Domain, extendFactor: number, isLog?: boolean): Domain
...
Compute the optimal size for a visualization to fill the available space given an optional aspect ratio.
...
Get the dimensions of a `ndarray` as a `{ rows, cols }` object.

```ts
getDims(dataArray: DataArray): Dims

const myArray = ndarray([0, 1, 2, 3], [1, 4]);
const dims = getDims(myArray); // { rows: 1, cols: 4 }
```
