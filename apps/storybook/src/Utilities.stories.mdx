...
- receives the `camera` object and the current `VisCanvasContext` value;
...
const htmlPt = useCameraState(
  (camera, context) => dataToHtml(camera, new Vector3(x, y)),
  [x, y, dataToHtml]
);
```
...
```ts
getVisibleDomains(camera: Camera, context: VisCanvasContextValue): {
...
Compute the center of the box => like [getCenter](https://threejs.org/docs/?q=box#api/en/math/Box3.getCenter) but much simpler to use.
...
`toRect` is unlikely to return those same two points.
