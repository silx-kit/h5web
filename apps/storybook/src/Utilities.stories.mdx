...
By default, React performs a strict equality check to avoid re-rendering the same state twice in a row. If your `factory` function returns an new object reference on every call,
you can avoid unnecessary re-renders by providing your own equality function. Note, however, that this optimisation is pointless if `factory` always returns a different state when the camera changes.

```ts
/* Once the user has zoomed past a certain threshold, `pt` is always (0, 0, 0),
 * so there's no point in re-rendering on every interaction any more. */
