...
| `getFovBox`      | Get a [`Box`](https://h5web-docs.panosc.eu/?path=/story/utilities--page#box) spanning the field of view of the camera in world space. The second parameter, `center`, defaults to `camera.position`. | <code>(camera: Camera, center?: Vector3) => Box</code> |
