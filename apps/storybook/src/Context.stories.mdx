...
| `worldToHtml`    | Converts a vector from world space to HTML space (combine with `useFrameRendering()` if called during render)      | <code>(vec: Vector2 &#124; Vector3) => Vector2</code> |
